...
const Search = ({ db }) => {
  ...
  return (
    <div className="app-main">
      {users && (
        <>
          <form className="p-3" onSubmit={searchUsers}>
              ...
              <button type="submit">
                {!searching && (
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="w-6 h-6 hover:text-primary transition duration-200"
                  >
                    <path
                      strokeLinecap="round"
                ...
                {searching && (
                  <svg
                    aria-hidden="true"
                    className=" w-6 h-6 text-gray-300 animate-spin dark:text-gray-600 fill-primary"
                    viewBox="0 0 100 101"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
      ...
      {users && (
        <div className=" border-b border-gray-200 mt-2 ">
          {result &&
            result.map((user) => (
              <Link to={`/profile/${uid}/${user.id}`} key={user.id}>
                <div className="flex items-center gap-2 p-4 border-t border-gray-200 hover:bg-gray-100 cursor-pointer">
          ...
          {result.length === 0 && (
            <div className=" text-2xl text-gray-300 p-4 flex items-center justify-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
